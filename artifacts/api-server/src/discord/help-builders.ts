    ...
    embed.addFields(
      {
        ...
        value: [
          ...
          `\`${p}ai remember <info>\` · \`${p}ai memory\` · \`${p}ai forget <n|all>\` — ` + { en: "optional private memory", fr: "mémoire privée facultative", es: "memoria privada opcional", de: "optionaler privater Speicher", pt: "memória privada opcional", it: "memoria privata opzionale" }[tl],
          `\`${p}ai server remember <info>\` — ` + { en: "admin-only shared server facts", fr: "fait commun réservé aux admins", es: "dato común solo para admins", de: "gemeinsame Admin-Fakten", pt: "facto comum para admins", it: "fato comum para admins" }[tl],
  ...
  switch (topic) {
    ...
    case "ai":
      ...
      embed.addFields(
        ...
        { name: `\`${p}ai remember\` / \`${p}ai memory\` / \`${p}ai forget\``, value: { en: "Opt-in encrypted personal memory. Memory is sent privately when viewed.", fr: "Mémoire personnelle chiffrée avec accord. La mémoire est envoyée en privé.", es: "Memoria personal cifrada con consentimiento. Se muestra por privado.", de: "Verschlüsselter persönlicher Speicher mit Zustimmung.", pt: "Memória pessoal criptografada com consentimento.", it: "Memoria personale crittografata con consenso." }[tl] ?? "" },
