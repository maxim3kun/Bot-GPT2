...
// ── Logo brand pool (grows dynamically at runtime) ───────────────────────────
let logoBrandPool: LogoBrand[] = [...HARDCODED_BRANDS];
let dynamicBrandsLoaded = false;
let dynamicLoadPromise: Promise<void> | null = null;

function triggerDynamicLoad(): void {
  ...
  dynamicLoadPromise = loadBrandsWithCache(token).then((brands) => {
    ...
    logger.warn({ err }, "Dynamic logo brand load failed — using hardcoded list only");
  });
}

// fire dynamic load on module init
triggerDynamicLoad();
...
const LOGO_DIFF_CONFIG: Record<LogoDifficulty, {
  tiers: number[];
  maxHints: number;
  timeMs: number;
  label: string;
...
async function runLogoGame(channel: DiscordChannel, channelId: string, diffArg?: string): Promise<void> {
  ...
  if (!pickLogoToken()) {
    await channel.send("❌ Logo game is not configured. Ask a moderator to set `LOGO_DEV_PUBLIC_KEY` — use `!mode d'emploi` for instructions.");
    return;
  }
